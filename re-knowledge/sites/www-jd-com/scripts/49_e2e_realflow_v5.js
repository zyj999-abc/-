#!/usr/bin/env node
/**
 * 京东登录 - 真实流程 v5 (强制调用 smartInitSlide)
 *
 * 关键发现:
 *   - 真实 login2024.js 调 initJdSlide 的条件是 useSlideAuthCode=true
 *   - 我们直接调 window.smartInitSlide() 强制触发
 *   - 真实参数: id="loginsubmit", scene="login", product="bind-suspend"
 */
...
(async () => {
  ...
  console.log(`[REALFLOW-V5] USERNAME: ${USERNAME}`);
  ...
  page.on('console', (msg) => {
    const t = msg.text();
    if (t.includes('jdSlide') || t.includes('slider_pass') || t.includes('error') || t.includes('smartInit')) {
  ...
  console.log('\n[5] 强制调用 window.smartInitSlide()...');
  const initResult = await page.evaluate(() => {
    if (typeof window.smartInitSlide === 'function') {
      try { window.smartInitSlide(); return 'called'; }
  ...
  for (let i = 0; i < 40; i++) {
    ...
    btnInfo = await page.evaluate(() => {
      ...
      return { x: r.x, y: r.y, w: r.width, h: r.height, jdSlideExists: !!window.jdSlide };
    });
    if (btnInfo && btnInfo.w > 0 && btnInfo.h > 0) {
      console.log(`  btn 出现 (i=${i}s): ${JSON.stringify(btnInfo)}`);
      break;
    }
    if (i % 5 === 0) console.log(`  i=${i}s btnInfo: ${JSON.stringify(btnInfo)}`);
  }

  if (!btnInfo) {
    console.log('  没出 btn, 看 page state');
    const state = await page.evaluate(() => ({
      loginsubmitHTML: document.querySelector('#loginsubmit') ? document.querySelector('#loginsubmit').outerHTML.slice(0, 500) : null,
      jdSlideExists: !!window.jdSlide,
      jdSlideType: typeof window.jdSlide,
      allJdsClasses: Array.from(document.querySelectorAll('[class*=JDJRV], [class*=jd]')).map(e => e.className).filter(c => c).slice(0, 10),
    }));
    console.log(JSON.stringify(state, null, 2));
    await page.screenshot({ path: '/tmp/jd_track/e2e_v5_no_btn.png', fullPage: true });
    await browser.close();
    return;
  }
  ...
  console.log(`  gData: y=${gData ? gData.y : 'null'}, patch=${gData ? gData.patch.length : 0} chars`);
  ...
  console.log(`\n[9] loginService 响应 (${loginResps2.length} 条):`);
  ...
  console.log(`\n[10] cookies (${useful.length}/${cookies.length}):`);
  for (const c of useful) {
    console.log(`  ${c.name}: ${c.value.slice(0, 60)}`);
  }

  await page.screenshot({ path: '/tmp/jd_track/e2e_v5_final.png', fullPage: true });

  fs.writeFileSync('/tmp/jd_track/e2e_v5_resps.json', JSON.stringify(respLog, null, 2));
